...
void Creatdat(char *s)//初始化100条记录存放在community
{
	FILE *fPtr = NULL;
...
void outputlist(int list,char *s)
{
	FILE *f = NULL;
	struct product pro;
	if (f = fopen(s, "rb"))
...
void Delete_record(char *s)//删除一个节点
{
	int ID;
	struct product pro;
...
int main()
{
	...
	while (flag != 6)
	{
		switch (flag)
		{
		case 1: {Creatdat(s); scanf("%d", &flag); return; }
		case 2: {First_Write(s); scanf("%d", &flag); return; }
		case 3: {Update_record(s); scanf("%d", &flag); return; }
		case 4: {Delete_record(s); scanf("%d", &flag); return; }
		case 5: {Outputalllist(s); scanf("%d", &flag); return; }
		}
